...
if (moduleJson.version !== "0.5.0") fail("experimental module version should be 0.5.0"); else ok("module version 0.5.0");
...
const styles = fs.readdirSync(path.join(ROOT, "styles")).filter((f) => f.endsWith(".css")).map((f) => fs.readFileSync(path.join(ROOT, "styles", f), "utf8")).join("\n");
...
for (const requiredClass of ["crnsl-commandbar","crnsl-sessionbar","crnsl-action-dock","crnsl-inspector-tabs","crnsl-program-drawer","reduced-motion","crnsl-v05-encounter","crnsl-v05-pursuit"]) {
  if (!styles.includes(`.${requiredClass}`)) fail(`CSS component missing: ${requiredClass}`);
}
if (!styles.includes(":focus-visible")) fail("keyboard focus treatment missing");
if (!styles.includes("prefers-reduced-motion")) fail("prefers-reduced-motion support missing");
if (!failures) ok("cockpit component/accessibility CSS");

for (const [label, file] of [
  ["pure RED rules regression suite", "test-rules.mjs"],
  ["0.3 UX baseline suite", "test-v03.mjs"],
  ["0.4 motion baseline suite", "test-v04.mjs"],
  ["0.5 Black ICE/Slide suite", "test-v05.mjs"]
]) {
  const result = spawnSync(process.execPath, [path.join(ROOT, "tools", file)], { encoding: "utf8" });
  process.stdout.write(result.stdout || ""); process.stderr.write(result.stderr || "");
  if (result.status !== 0) fail(label); else ok(label);
}
